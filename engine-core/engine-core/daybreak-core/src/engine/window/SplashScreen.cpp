...
#define WM_OUTPUTMESSAGE (WM_USER + 0x0001)
	...
	void Open() {
		...
		m_SplashWindow->Show();
		Logger::debug(L"Created SplashScreen\n");
	}
	...
	void Close() {
		if (m_SplashWindow != nullptr) {
			...
			Logger::debug(L"Destroyed SplashScreen\n");
			return;
		}
		Logger::error(L"Call to Close() before SplashScreen created!\n");
	}
